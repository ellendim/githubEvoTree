...
for (x in file_list_1){
  if (file.exists(x)){ 
    ...
    filter(Max.p.val < pval) # only p-values under ...
  # group_by(OrthoGroup) %>%
  ...
  scaled_data_s1 <- t(scale(t(s1_fs), center = T, scale = TRUE))
  scaled_data_s1[is.nan(scaled_data_s1)] <-0
    ...
    for (i in file_list_2){ # Go through the EXPRESSION DATA...again.
      if (grepl(key_word_s2, i)){ # If the string above contains key_word_2 ...
        ...
        scaled_data_s2 <- t(scale(t(s2_fs), center = T, scale = TRUE))
        scaled_data_s2[is.nan(scaled_data_s2)] <-0
      
      }}

    corr_matrix <- round(cor(scaled_data_s1, scaled_data_s2, method = "pearson"), 4) # Correlation matrix using the filtered + sorted
...
for(matrix in corr_list){
    ...
    col_function_1 <- colorRamp2(c(lowest,  between, highest), c("#08306b", "#f7f7f7","#de2d26"))
   # col_function_3 <- colorRamp2(c(lowest,  between, highest), c(   "gold", "white","forestgreen"))  c(   "#440154", "#31688e","#fde725")) 
