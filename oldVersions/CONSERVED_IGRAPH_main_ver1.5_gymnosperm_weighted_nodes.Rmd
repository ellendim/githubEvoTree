...
count_df <- data.frame(
  ...
  OrthologGroups = c(687, 1030, length(unique(ultra_conserved_gymno$OrthologGroup)), length(unique(partial3_conserved_gymno$OrthologGroup)))
...
ggplot(count_df, aes(x = conserved, y = OrthologGroups, fill = conserved)) +
  ...
  theme_minimal() +  theme(legend.position = "right", axis.text.x = element_blank(), axis.title.x = element_blank(),panel.background = element_rect(fill = "cornsilk"), plot.background = element_rect(fill = "white")) + ggtitle("Conserved ortholog groups specific for Gymnosperms") + ylim(0,1200) + geom_hline(aes(yintercept = 1195, linetype = "Max cliques possible")) + scale_linetype_manual(values = 2)
