...
expressolog_file <- read.delim("Data/DATA/textFiles/cons_genes_all_species.txt", sep = " ") %>% 
  mutate(SpeciesPair = paste0(Species1, Species2)) %>% 
  filter(MaxPVal < p_thr) %>% 
  filter(OrthologGroup %in% ultraConservedOrthoGroups)
# We are working with the same ortholog groups
