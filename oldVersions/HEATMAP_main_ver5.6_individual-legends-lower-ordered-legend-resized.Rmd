...
for(matrix in corr_list){
   ...
   hm <-  Heatmap(get(matrix),show_column_names = FALSE, 
    ...
    column_order = 1:ncol(get(matrix)), col = col_function_3, heatmap_legend_param = list(title = "Cor", title_gp = gpar(fontsize = 20) , legend_height = unit(10, "cm"), grid_width = unit(0.7, "cm"), labels_gp = gpar(fontsize = 20), title_gap =  unit(2, "cm"))) 
  
    # Gather the max values to plot
  reference_matrix <- rbind(reference_matrix,
  data.frame(
  S1 = key_word_s1,
  S2 = key_word_s2,
  max_value = highest
...
legend_matrix <- reference_matrix %>% 
  pivot_wider()
