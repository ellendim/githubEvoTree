...
# Identifying complete and partial cliques amongst angiosperms pairs
...
# save(cons_og_red, file = "Data/Data/conserved_orthogroups_angios_reduced.RData")
...
# save(clique_genes_unlisted_prefilter, file = "Data/DATA/clique_genes_angiosperms_prefiltered.RData")
...
complete_cliques_angio <- weighted_max_cliques_filtered %>% 
  filter(cliqueSize == 3) 

length(unique(complete_cliques_angio$OrthologGroup))


# Partial cliques across all pairs (can be modified)

 partial_cliques_angio <- weighted_max_cliques_filtered %>% 
   filter(cliqueSize >= 2)
length(unique(partial_cliques_angio$OrthologGroup))
...
# Complete cliques
species_1 <- complete_cliques_angio%>% 
  select(c(1,7,11))%>% 
  rename(Species = "UGeneSpecies1")

species_2 <- complete_cliques_angio %>% 
  select(c(1,7,12)) %>% 
  rename(Species = "UGeneSpecies2")


complete_cliques_angio_listed <- rbind(species_1, species_2)

complete_cliques_angio_listed <- complete_cliques_angio_listed %>% 
  as.data.frame() %>% 
  group_by(cliqueID) %>% 
...
length(unique(complete_cliques_angio_listed$OrthologGroup))

# Partial cliques

species_1 <- partial_cliques_angio%>% 
  select(c(1,7,11))%>% 
  rename(Species = "UGeneSpecies1")

species_2 <- partial_cliques_angio %>% 
  select(c(1,7,12)) %>% 
  rename(Species = "UGeneSpecies2")


partial_cliques_angio_listed <- rbind(species_1, species_2)

partial_cliques_angio_listed <- partial_cliques_angio_listed %>% 
  as.data.frame() %>% 
  group_by(cliqueID) %>% 
  distinct(Species, .keep_all = T) %>% 
  ungroup() %>% 
...
length(unique(partial_cliques_angio_listed$OrthologGroup))


# Save

save(complete_cliques_angio_listed, file = "Data/DATA/complete_cliques_angios.RData")
save(partial_cliques_angio_listed, file = "Data/DATA/partial_cliques_angios.RData")
