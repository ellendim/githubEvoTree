...
```{r}
setwd("~/NMBU/M60-BIAS/EvoTree/githubEvoTree/GO_enrichment")
...
# Expressed genes
expressed_genes <- read_delim("Data/DATA/AspWood_transcriptomics.txt")
...
k <- read_delim("Data/DATA/cons_genes_aspen.txt", "\t")

cat("Number of expressed genes: ", length(unique(expressed_genes$Genes)))
cat("Number of expressed genes with GO-term : ", length(unique(go_list$`Sequence Name`)))
...
list <- c("Data/DATA/cons_genes_aspen.txt", "Data/DATA/gymnosperms_genes_aspen.txt", "Data/DATA/angiosperms_genes_aspen.txt")
...
for (i in 1:length(list)){
  ...
  group_name <- sapply(strsplit(list[i], "DATA/"), "[", 2)
  ...
  # Rename the summary table!
    ...
    new_name <- as.character(paste0("summary_table_", group_name))
