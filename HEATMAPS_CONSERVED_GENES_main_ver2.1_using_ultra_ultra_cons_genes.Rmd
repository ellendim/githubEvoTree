...
for (i in 1:length(file_list_2)){
  ...
  uuc_expr <- x %>% 
    filter(Genes %in% conserved_genes$Genes)
...
column_split <- rep("A", 158)
column_split[26:53] <- "B"
column_split[54:80] <- "C"
column_split[81:103] <- "S"
column_split[104:130] <- "T"
column_split[132:158] <- "L"
...
hm <-  Heatmap(combined_expression_matrix, show_column_names = TRUE,
               ...
               width = ncol(combined_expression_matrix)*unit(5, "mm"),
               height = nrow(combined_expression_matrix)*unit(2, "mm"),
