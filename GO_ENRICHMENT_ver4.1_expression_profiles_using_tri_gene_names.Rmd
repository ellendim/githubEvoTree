...
ortholog_group_file <- read.delim("Data/DATA/Orthogroups.100323.tsv", header = TRUE, sep = "\t") %>% mutate(Pinsyl.SHORT.cp.pep = Pinsyl.SHORT.pep) %>% 
  ...
  filter(Asp == "Potra2n6c14105")

print(ortholog_group_file$Tri)
...
genes_to_all_genes_filtered <- genes_to_all_genes %>% 
  separate_rows(Asp, sep = ", ")%>% 
  mutate(Asp = gsub("Poptre_", "", Asp)) %>%
  mutate(Asp = gsub("\\.\\d\\.p\\d$", "", Asp))%>% 
  filter(Asp %in% uuc_genes_asp$Genes)
unique(genes_to_all_genes_filtered$Tri)
unique(genes_to_all_genes_filtered$Asp)
...
unique(genes_to_all_genes$Tri)
unique(genes_to_all_genes$Asp)
