...
for (file in ortholog_group_RData) {
  ...
  # Now we want to filter ortho using each species expression/transcription data.
    ...
    ortho_filtered <- ortho_filtered[1:500]

    species1_expr_data <- species1_expr_data[species1_expr_data$Genes %in% ortho_filtered$Species1,]
    species2_expr_data <- species2_expr_data[species2_expr_data$Genes %in% ortho_filtered$Species2,]
...
# For each species pair (combo) we need to make a comparison file.

for (i in 1:nrow(combo)) {
  s1 <- combo[i, "X1"]
  s2 <- combo[i, "X2"]
   
  comparison_RData <- paste0("RData/comparison-", s1, "-", s2, "-", 
    ...
    # Expression data for species 1
    species1_transcription_txt <- paste0("Data/DATA/", s1,"Wood_transcriptomics.txt")
    species2_transcription_txt <- paste0("Data/DATA/", s2,"Wood_transcriptomics.txt")
  # Expression data for species 2
  ...
  # }
    ...
    ortho_s1_s2 <- paste0("ortho_",s1, "-", s2)
