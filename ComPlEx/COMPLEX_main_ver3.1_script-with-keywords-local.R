...
if (!file.exists(file)){ 
  ...
  species1_transcription_txt <- paste0("Data/DATA/", species1_keyword,"Wood_transcriptomics.txt")
  species2_transcription_txt <- paste0("Data/DATA/", species2_keyword,"Wood_transcriptomics.txt")
  ...
  if (!file.exists(comparison_RData)) {
    ...
    if (norm_method == "CLR") {
      ...
      species1_net <- sqrt(R * t(R)) # Do some maths...gives symmetric matrix without changing values?
    ...
    diag(species2_net) <- 0
     ...
     # When doing a test run the number of rows will be equal, so R will be the same!
    ...
    for (i in 1:nrow(ortho)) {
      ...
      neigh <- species1_net[ortho$Species1[1],]  
      neigh <- names(neigh[neigh >= species1_thr]) 
      
      ortho_neigh <- species2_net[ortho$Species2[1],] 
